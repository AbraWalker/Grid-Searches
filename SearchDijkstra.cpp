...
bool compareCost(unique_ptr<SNode>& lhs, unique_ptr<SNode>& rhs)
{
	return lhs->cost < rhs->cost;
}

void CSearchDijkstra::addNewNode(TerrainMap& terrain, NodeList& openList, NodeList& closedList, bool& onList, unique_ptr <SNode>& current, unique_ptr <SNode>& goal, int xModifier, int yModifier)
{
...
bool CSearchDijkstra::FindPath(TerrainMap& terrain, unique_ptr<SNode> start, unique_ptr<SNode> goal, NodeList& path)
{
	...
	do
	{
		...
		if (openList.size() > 1)
		{
			sort(openList.begin(), openList.end(), compareCost);
			timesSorted++;
		}
